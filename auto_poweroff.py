...
def get_remote_uptime_file(remote_host, remote_pass):
    ...
    os.system(
        "sshpass -p " + remote_pass + " ssh " + remote_host + ' "sshpass -p ' + remote_pass + ' sudo cp /proc/uptime ' +
        remote_uptime_file[0] + remote_host.split('@')[0] + remote_uptime_file[1] + ' "')
    time.sleep(3)
    os.system(
        "sshpass -p " + remote_pass + " scp " + remote_host + ":" + remote_uptime_file[0] + remote_host.split('@')[0] +
        remote_uptime_file[1] + " " + local_copy_remote_uptime_file)
